...
std::shared_ptr<Texture> TextureManager::createTexture(std::string textureName)
{
    ...
    if (textureID == 0) {
        // Pokud se načtení nepodařilo, nastavíme výchozí texturu
    ...
    // Vytvoř novou instanci textury a přidej ji do mapy
...
void TextureManager::deleteTextureByName(const std::string& textureName)
{
    ...
    // Lambda podmínka pro odstranění prvků rovné dataToDelete
