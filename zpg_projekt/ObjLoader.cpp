...
bool loadAssImp(
	...
	if (!scene) {
		...
		if (mat->GetTexture(aiTextureType_DIFFUSE, 0, &name) == AI_SUCCESS) {
			material.diffuseMap = name.C_Str();
		}
		if (mat->GetTexture(aiTextureType_SPECULAR, 0, &name) == AI_SUCCESS) {
			material.specularMap = name.C_Str();
		}
		if (mat->GetTexture(aiTextureType_HEIGHT, 0, &name) == AI_SUCCESS) {
			material.heightMap = name.C_Str();
		}

		int texIndex = 0;
		if (mat->GetTexture(aiTextureType_DIFFUSE, texIndex, &name) == AI_SUCCESS)
		{
			std::string sDir = path;
			std::string sTextureName = name.data;
